  ...
  security-scan:
    ...
    steps:
      ...
      - name: Run Security Scan
        run: |
          forticore scan --target https://\${{ github.repository }}.example.com --output json --output-file scan-results.json
      
      - name: Check for High Severity Issues
        run: |
          vulnerabilities=$(cat scan-results.json | jq '.vulnerabilities | length')
          high_severity=$(cat scan-results.json | jq '.vulnerabilities[] | select(.severity == "high") | length')
          echo "Found \$vulnerabilities vulnerabilities (\$high_severity high severity)"
