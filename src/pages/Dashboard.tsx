        ...
        {messages.length === 0 ? (
          <div className="flex-1 flex items-center justify-center px-4 py-8 overflow-y-auto">
            <div className="max-w-4xl w-full">
              ...
              <div className="mb-8 rounded-xl border border-blue-300 dark:border-blue-500/30 bg-gradient-to-br from-blue-50/50 to-purple-50/50 dark:from-blue-950/20 dark:to-purple-950/20 backdrop-blur-sm overflow-hidden">
                <div className="px-6 py-4 bg-blue-100/50 dark:bg-blue-950/40 border-b border-blue-300 dark:border-blue-500/30">
                  <div className="flex items-center gap-3">
                    <Terminal className="h-5 w-5 text-blue-600 dark:text-blue-400" />
                    <h3 className="text-sm font-bold text-blue-700 dark:text-blue-300 uppercase tracking-wider">FortiCore CLI Reference</h3>
                  </div>
                  <p className="text-xs text-gray-600 dark:text-gray-400 mt-1 font-mono">Comprehensive command syntax for the FortiCore penetration testing tool</p>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {[
                        { cmd: 'fortc scan -t example.com', desc: 'Basic scan on a target' },
                        { cmd: 'fortc scan -t 192.168.1.1 -s network -o scan-results.json', desc: 'Network scan with output file' },
                        { cmd: 'fortc scan -t https://example.com -s web -v', desc: 'Web application scan in verbose mode' },
                        { cmd: 'fortc scan -t example.com -s web --scan-subdomains', desc: 'Web scan with subdomain discovery' },
                        { cmd: 'fortc scan -t 192.168.1.100 -s vuln -v', desc: 'Vulnerability scan for IP targets' },
                        { cmd: 'fortc scan -t example.com -s ssl -v', desc: 'SSL/TLS configuration analysis' },
                        { cmd: 'fortc scan -t example.com -s full -o scan-results.json', desc: 'Full comprehensive port scan' }
                      ].map((item, i) => (
                        <div key={i} className="group relative">
                          <div className="flex items-start gap-3 p-3 rounded-lg bg-white/50 dark:bg-[#0d1220]/50 border border-gray-200 dark:border-purple-500/20 hover:border-purple-300 dark:hover:border-purple-500/40 transition-all hover:shadow-md dark:hover:shadow-[0_0_15px_rgba(168,85,247,0.2)]">
                            <code className="flex-1 text-xs font-mono text-gray-800 dark:text-gray-300 break-all">{item.cmd}</code>
                            <button
                              onClick={() => {
                                navigator.clipboard.writeText(item.cmd);
                              }}
                              className="opacity-0 group-hover:opacity-100 text-[10px] px-2 py-1 rounded bg-purple-100 dark:bg-purple-600/20 hover:bg-purple-200 dark:hover:bg-purple-600/30 text-purple-700 dark:text-purple-300 border border-purple-300 dark:border-purple-500/30 transition-all"
                            >
                              COPY
                            </button>
                          </div>
                          <p className="text-[10px] text-gray-500 dark:text-gray-500 mt-1 ml-3 font-mono"># {item.desc}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Exploit Commands */}
                  <div>
                    <h4 className="text-xs font-bold text-orange-700 dark:text-orange-300 uppercase tracking-wider mb-3 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-orange-500" />
                      Exploit Mode
                    </h4>
                    <div className="space-y-2">
                      {[
                        { cmd: 'fortc exploit -t example.com --vuln-id WEB-004', desc: 'Exploit specific vulnerability' },
                        { cmd: 'fortc exploit -t 192.168.1.1 --safe-mode true', desc: 'Discovery and exploitation in safe mode' },
                        { cmd: 'fortc exploit -t example.com --scan-file path/to/scan-results.json', desc: 'Use custom scan results file' },
                        { cmd: 'fortc exploit -t example.com -o exploit-results.json', desc: 'Save exploitation results to file' },
                        { cmd: 'fortc exploit -t example.com -o exploit-results.json --generate-report', desc: 'Generate PDF report of results' }
                      ].map((item, i) => (
                        <div key={i} className="group relative">
                          <div className="flex items-start gap-3 p-3 rounded-lg bg-white/50 dark:bg-[#0d1220]/50 border border-gray-200 dark:border-orange-500/20 hover:border-orange-300 dark:hover:border-orange-500/40 transition-all hover:shadow-md dark:hover:shadow-[0_0_15px_rgba(249,115,22,0.2)]">
                            <code className="flex-1 text-xs font-mono text-gray-800 dark:text-gray-300 break-all">{item.cmd}</code>
                            <button
                              onClick={() => {
                                navigator.clipboard.writeText(item.cmd);
                              }}
                              className="opacity-0 group-hover:opacity-100 text-[10px] px-2 py-1 rounded bg-orange-100 dark:bg-orange-600/20 hover:bg-orange-200 dark:hover:bg-orange-600/30 text-orange-700 dark:text-orange-300 border border-orange-300 dark:border-orange-500/30 transition-all"
                            >
                              COPY
                            </button>
                          </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {[
                        { cmd: 'fortc report -i scan-results.json -o security-report.pdf', desc: 'Generate PDF report from scan' },
                        { cmd: 'fortc report -i scan-results.json -o security-report.txt', desc: 'Generate text report from scan' },
                        { cmd: 'fortc report -i exploit-results.json -o exploit-report.pdf', desc: 'Generate report from exploitation' }
                      ].map((item, i) => (
                        <div key={i} className="group relative">
                          <div className="flex items-start gap-3 p-3 rounded-lg bg-white/50 dark:bg-[#0d1220]/50 border border-gray-200 dark:border-emerald-500/20 hover:border-emerald-300 dark:hover:border-emerald-500/40 transition-all hover:shadow-md dark:hover:shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                            <code className="flex-1 text-xs font-mono text-gray-800 dark:text-gray-300 break-all">{item.cmd}</code>
                            <button
                              onClick={() => {
                                navigator.clipboard.writeText(item.cmd);
                              }}
                              className="opacity-0 group-hover:opacity-100 text-[10px] px-2 py-1 rounded bg-emerald-100 dark:bg-emerald-600/20 hover:bg-emerald-200 dark:hover:bg-emerald-600/30 text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-500/30 transition-all"
                            >
                              COPY
                            </button>
                          </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div className="group relative">
                        <div className="flex items-start gap-3 p-3 rounded-lg bg-white/50 dark:bg-[#0d1220]/50 border border-gray-200 dark:border-cyan-500/20 hover:border-cyan-300 dark:hover:border-cyan-500/40 transition-all hover:shadow-md dark:hover:shadow-[0_0_15px_rgba(6,182,212,0.2)]">
                          <code className="flex-1 text-xs font-mono text-gray-800 dark:text-gray-300 break-all">source "/root/.cargo/env" && bash install.sh</code>
                          <button
                            onClick={() => {
                              navigator.clipboard.writeText('source "/root/.cargo/env" && bash install.sh');
                            }}
                            className="opacity-0 group-hover:opacity-100 text-[10px] px-2 py-1 rounded bg-cyan-100 dark:bg-cyan-600/20 hover:bg-cyan-200 dark:hover:bg-cyan-600/30 text-cyan-700 dark:text-cyan-300 border border-cyan-300 dark:border-cyan-500/30 transition-all"
                          >
                            COPY
                          </button>
                        </div>
                        <p className="text-[10px] text-gray-500 dark:text-gray-500 mt-1 ml-3 font-mono"># Run as root in the fortc directory (automatic installation)</p>
                      </div>
                    </div>
                  </div>

                  {/* Note */}
                  <div className="mt-4 p-4 rounded-lg bg-blue-100/50 dark:bg-blue-950/30 border border-blue-300 dark:border-blue-500/30">
                    <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">
                      <strong className="text-blue-700 dark:text-blue-300 font-mono">Note:</strong> These are CLI commands for the FortiCore penetration testing tool. 
                      The web interface above provides a subset of scanning capabilities through API endpoints. 
                      For full exploitation and reporting features, use the CLI tool.
                    </p>
                  </div>
                </div>
              </div>
