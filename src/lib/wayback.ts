...
export async function getWaybackUrlsOnly(targetUrl: string, limit = 100): Promise<string[]> {
  try {
    ...
    if (res.ok) {
      ...
      const data = await res.json();
      throw new Error(data.error || 'Wayback service temporarily unavailable');
    }
  } catch (error: any) {
    // Re-throw with more context
    if (error.message.includes('blocked') || error.message.includes('rate-limited')) {
      throw new Error(`Wayback access blocked: ${error.message}. Please try again later or use a different domain.`);
    }
    throw error;
  }
  
  // This should not be reached due to the API endpoint handling all cases
  throw new Error('Wayback service unavailable');
}
